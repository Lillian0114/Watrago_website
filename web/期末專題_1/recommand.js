...
var url_search = "&searchword=There%20is%20no%20keyword%20blablablablablablabla";
var url_countrysearch = "&countrysearch=There%20is%20no%20keyword%20blablablablablablabla";
var arraySimilar = "";
var id_array = "";
...
$("#testbtn").click(function (userid) {
    ...
    $('.form-check input:checked').each(function (index, val) {
        $('.form-check input:checked')[0].checked = false;
        // console.log($('.form-check input:checked'));
        // console.log($('.form-check input:checked')[0].checked);
    });
    //lillian
    count = 0;
    like_temp_count = 0;
    ...
    if (user_input !== "") {
        ...
        fetch(url + arraySimilar + "&searchword=" + user_input + url_countrysearch)
            .then(res => {
                return res.json();
            }).then(result => {
                search_array = result;
...
function load_similar(userid) {
    ...
    db.collection("user").doc(userid).get().then(function (docu) {
        if (arraySimilar == "") {
            ...
            // console.log(url + arraySimilar + url_search);
        }
        fetch(url + arraySimilar + url_search + url_countrysearch)
            .then(res => {
                return res.json();
            }).then(result => {
...
//------城市搜尋-----------green-----------
var IsClick = false;
function location_click() { //案放大鏡
    // console.log("222222334");
    IsClick = true;
    $(".location_search").css("display", "inline-block")
    $(".location_search_btn").css("top", "130px")
    $(".location_search_btn").css("animation", "body_drift 2s infinite linear")
    $(".location_search_btn").css("opacity", "1")
    $("#btn_ok").css("display", "inline-block")
    $("#btn_ok").css("top", "130px")
    $("#btn_ok").css("animation", "body_drift 2s infinite linear")

}

var country = ["台北市", "新北市", "基隆市", "桃園市", "新竹市", "新竹縣", "苗栗縣", "台中市", "彰化縣", "南投縣", "雲林縣", "嘉義市", "嘉義縣", "台南市", "高雄市", "屏東縣", "宜蘭縣", "花蓮縣", "台東縣"];
var selected_country = [];
function location_check() { //案ok
    // console.log("1234567");
    IsClick = false;
    $(".location_search").css("display", "none")
    $("#btn_ok").css("display", "none")
    $(".location_search_btn").css("top", "initial")
    $("#btn_ok").css("top", "initial")

    $('.form-check input:checked').each(function (index, val) {
        // console.log($(this).attr('value'));
        selected_country.push(country[$(this).attr('value') - 1]);
    });
    // console.log(selected_country);

    load_country_db(selected_country);
}

function load_country_db(selected_country) {
    userid = "MqiXBeBw2oMiy4uBrpx3";
    if (selected_country.length > 0) {
        ...
        for (var i = 0; i < selected_country.length; i++) {
            arraycountry = selected_country[i] + "," + arraycountry;
        }
        arraycountry = arraycountry.substring(0, arraycountry.length - 1);
        // console.log(arraycountry);
        // console.log(arraySimilar);
        // console.log(url + arraySimilar + "&countrysearch=" + arraycountry + url_search);
        fetch(url + arraySimilar + "&countrysearch=" + arraycountry + url_search)
            .then(res => {
                return res.json();
            }).then(result => {
                search_array = result;
                ...
                // id_array = result.SimilarID;
                for (var j = 0; j < search_array.length; j++) {
                    var first = db.collection("Attraction").doc(search_array[j]);
                    // console.log(search_array[j]);
                    first.get().then(function (doc) {
                        var docRef = db.collection("user").doc(userid);
                        docRef.get().then(function (fun_doc) {
                            like_list = fun_doc.data().Collection;
